...
AniRawData SceCells::obtainAniRawDataGivenCellColor(vector<double>& cellColors,
	...
	thrust::host_vector<double> hostTmpVectorFcellsubX(maxActiveNode);
	thrust::host_vector<double> hostTmpVectorFcellsubY(maxActiveNode);
	...
	thrust::copy(
			...
			thrust::make_zip_iterator(
					thrust::make_tuple(
							...
							hostActLevel.begin(),hostAdhNodeIndex.begin(),hostMinToAdhDist.begin(),hostNodePolar.begin(),hostTmpVectorFcellsubX.begin(),
							hostTmpVectorFcellsubY.begin()
							)));
	...
	for (uint i = 0; i < activeCellCount; i++) {
		...
		for (uint j = 0; j < curActiveMemNodeCounts[i]; j++) {
			...
			if ( hostIsActiveVec[index1]==true) {
				...
				tempFcellsub = CVector(hostTmpVectorFcellsubX[index1],hostTmpVectorFcellsubY[index1],0.0);
				rawAniData.Fcellsub.push_back(tempFcellsub);
...
VtkAnimationData SceCells::outputVtkData(AniRawData& rawAniData, 
	...
	for (uint i = 0; i < rawAniData.aniNodePosArr.size(); i++) {
		...
		ptAniData.Fcellsub1= rawAniData.Fcellsub[i];
...
void SceCells::calSceCellMyosin2() {
	totalNodeCountForActiveCells = allocPara_m.currentActiveCellCount
			* allocPara_m.maxAllNodePerCell;
	uint maxAllNodePerCell = allocPara_m.maxAllNodePerCell;
	uint maxMemNodePerCell = allocPara_m.maxMembrNodePerCell;
...
/*
void SceCells::updateCellPolarLeader2() {
	cout<< "I am starting to compute interior node polarity "<< endl;
	totalNodeCountForActiveCells = allocPara_m.currentActiveCellCount
			* allocPara_m.maxAllNodePerCell;
	uint maxAllNodePerCell = allocPara_m.maxAllNodePerCell;
	uint maxMemNodePerCell = allocPara_m.maxMembrNodePerCell;
	...
	thrust::transform(
			...
			thrust::make_zip_iterator(
					thrust::make_tuple(nodes->getInfoVecs().nodeVelX.begin(),
							   nodes->getInfoVecs().nodeVelY.begin()
							   //nodes->getInfoVecs().subAdhLocX.begin(),
