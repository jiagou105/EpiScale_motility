...
struct updateCellPolarDevice: public thrust::unary_function<UUUIIDDDD, double> {
	...
	__device__ double operator()(const UUUIIDDDD &cData) const {
		...
		double filopTipY;
		double distCells;
		double randNoiseAngle = u01(rng)*2*3.14159;
		// double PI = acos(-1.0);
		uint nodeIndex;
		...
		if (_timeNow < 55800.0) {
			...
			if (_cellTypeAddr[_leaderRank]==1){ // means a leader exists
				...
				cellAngle = randNoiseAngle;//cellAngle + _timeStep*rateRep*(allPolar+randNoiseAngle); //u01(rng)*2*3.1415926;// 
			}
			return cellAngle;
		}
	}
				...
				for (int allNbrIndex = 0; allNbrIndex < (maxSubSitePerNode-bindSiteCounts); allNbrIndex++){
					...
					//} else{
						if (cellType==1){
							...
							siteBindThreshold = 0.4; // means follower
						}
					// }
