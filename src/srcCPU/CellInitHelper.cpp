...
//			mesh.getAllInsidePoints();
	...
	for (uint i = 0; i < initCellCt; i++) {
		randNum = 1 * progDivStart; // (double)rand() / ((double)RAND_MAX + 1) * progDivStart;
...
vector<CVector> CellInitHelper::generateInitIntnlNodes(CVector& center,
	...
	uint maxInitNodeCountFollower = globalConfigVars.getConfigValue(
		"MaxIntnlNodeCountPerFollower").toInt();
	//Ali

	//	uint initIntnlNodeCt = minInitNodeCount ; 
	uint initIntnlNodeCt = minInitNodeCount + (maxInitNodeCountFollower - minInitNodeCount) * initProg; 
